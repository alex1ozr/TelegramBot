// Я пишу статью по разработке Telegram бота на .NET, основываясь на текущем солюшене.
# Разработка ботов для Telegram на платформе .NET

## Введение
Вряд ли [Telegram](https://telegram.org/) нуждается в представлении, так как это один из популярнейших мессенджеров в мире,
предлагающий множество функций, включая групповые чаты, каналы, голосовые и видеозвонки, а также возможность создания ботов.

В данной статье мы не будем ставить целью показать, как создать бота a-la "Hello, World!" с нуля,
но рассмотрим более сложный пример готовой реализации бота на платформе .NET с использованием современных технологий и практик разработки.

## Выбор библиотеки для создания бота
Существует несколько способов создания ботов для Telegram на платформе .NET, включая использование сторонних библиотек и фреймворков.
...
- Постоянно обновляется и поддерживается сообществом.
- Имеет достаточно обширную документацию и примеры использования.
...
- Так же как и Telegram.Bot, поддерживает все возможности Telegram Bot API, получает регулярные обновления и имеет хорошие примеры использования.
...
- Третий по популярности вариант библиотеки для создания Telegram ботов на платформе .NET.
- Предоставляет простой и относительно удобный интерфейс для создания ботов.
- Разработка с использованием данной библиотеки напоминает разработку приложений на Windows Forms.
...
Для примера реализации была выбрана библиотека [Telegram.BotAPI](https://github.com/Eptagone/Telegram.BotAPI),
так как она обладает всеми актуальными возможностями и предоставляет **лучшие** примеры использования, нежели библиотека [Telegram.Bot](https://github.com/TelegramBots/Telegram.Bot).
Данный выбор - сугубо субъективный и не является рекомендацией в пользу одной из библиотек, так как обе библиотеки одинаково хороши и схожи по использованию.
...
Давайте рассмотрим непосредственно [GitHub-репозиторий](https://github.com/alex1ozr/TelegramBot) с примером реализации.
...
- Функциональные:
    ...
    - Поддержка пожертвований в [Telegram Stars](https://telegram.org/blog/telegram-stars)
...
#### Общая структура
Решение состоит из нескольких слоев, каждый из которых отвечает за свою функциональность. Основные слои включают:
...
### Обработка команд
Ключевым элементом любого Telegram бота является обработка сообщений/команд, которые пользователи отправляют боту для выполнения определенных действий.
...
## Итоги
Таким образом, рассмотренный выше пример реализации Telegram бота на платформе .NET использует современные технологии и практики разработки.
Дальнейшее расширение функционала бота заключается лишь в добавлении новых команд и их обработчиков, с сохранением чистоты и модульности кода.
